...
public class BinarniBroj implements Comparable<BinarniBroj> {
	...
	public String hexZapis(Map<String, String> kodovi) {
		...
		StringBuilder ceoDeoSB = new StringBuilder(ceoDeo);
		StringBuilder razlomljeniDeoSB = new StringBuilder(razlomljeniDeo);
		...
		for(int i = 0; i <= ceoDeoSB.length()-4; i += 4) 
			rezultat.append(kodovi.get(ceoDeoSB.substring(i, i+4)));

		/* konverzija razlomljenog dela */
		if(!razlomljeniDeo.equals("")) {
			rezultat.append(".");
			for(int i = 0; i <= razlomljeniDeoSB.length()-4; i += 4) 
				rezultat.append(kodovi.get(razlomljeniDeoSB.substring(i, i+4)));
		}
	...
	/* Ova varijanta bi bila pogresna, jer se za 
	 * realne brojeve koji imaju isti ceo deo dobija
	 * rezultat 0.
	 */
//	public int compareTo(BinarniBroj o) {
//		return (int)(this.dekVr() - o.dekVr());
//	} 
	
	@Override
	public String toString() {
		if(!razlomljeniDeo.equals(""))
			return ceoDeo + "." + razlomljeniDeo;
		return ceoDeo;
	}
}
